"""
Standalone Brand Blueprint FastAPI Application
This file contains all the routes and logic for the brand blueprint feature.
Can be used as a standalone app or integrated into another application.
"""

from fastapi import FastAPI, Depends, HTTPException, Query, Request, Body, UploadFile, File
...
# Import from standalone modules (adjust imports based on your structure)
...
app = FastAPI(
    title="Brand Blueprint API",
    description="AI-powered brand blueprint management with PDF processing",
    version="1.0.0",
    lifespan=lifespan
)
...
@app.post("/api/brand/upload-guideline")
...
@app.post("/api/brand/blueprint/approve")
...
@app.get("/brand-blueprint", response_class=HTMLResponse)
